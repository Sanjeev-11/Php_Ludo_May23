...
if (isset($_POST['rollButton'])) {
    ...
    if ($gameState[8] == 1 || $gameState[8] == 2) {
        ...
        $gameState = array_fill(0, 9, 0);
        $gameState[0] = 12;
        echo '<script>alert("Player ' . $winner . ' is the wins! and the dice value was ' . $diceValue . '.");</script>';
        $gameState['currentPlayer'] = 1;
        $gameState['currentDice'] = 0;
        session_unset();
        session_destroy();
...
<body>
    ...
    <div id="game-board">
        ...
        if ($gameState['currentDice'] != 0) {
            ...
            echo 'Player ' . $player . ' rolled a ' . $gameState['currentDice'] . '!';
        }
        ?>
    </div>
    <form method="post">
